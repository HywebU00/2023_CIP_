...
const getData = {
  async fetch(apiurl, condForm) {
    return new Promise(resolve => {
      setTimeout(() => {
        axios.post("/api" + apiurl, condForm)
        ...
        <div class="searchGrp" style="width:90%">
          <div class="searchList">
            <v-container>
              <v-row>
                <v-col cols="4">
                  <v-text-field label="帳號、姓名或E-mail" variant="outlined"
                    v-model="pageInfo.searchForm.fkeyword"></v-text-field>
                </v-col>
                <v-col cols="4">
                  <v-select label="狀態" variant="outlined" v-model="pageInfo.searchForm.fstatus"
                    :items="pageInfo.condition['statusList']" item-title="text" item-value="value"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-btn elevation="0">查詢</v-btn>
                  <v-btn class="searchMoreBtn" elevation="0"
                    @click="pageInfo.searchForm.expand = !pageInfo.searchForm.expand">進階搜尋</v-btn>
                </v-col>
              </v-row>
          ...
          <div class="searchMoreContent">
            <v-expand-transition>
              <v-card v-show="pageInfo.searchForm.expand" class="">
                    ...
                    <v-row>
                      <v-col cols="12" md="6">
                        <div class="d-flex formGrp">
                          ...
                          <v-autocomplete variant="solo" single-line hide-details density="compact"
                            :items="['1990', '1991', '1992']"></v-autocomplete>
                          <span class="tilde">~</span>
                          <v-autocomplete variant="solo" density="compact" single-line hide-details
                            :items="['1990', '1991', '1992']"></v-autocomplete>
                        </div>
                        <div class="d-flex formGrp">
                          <label class="formTitle" for="">類別 :</label>
                          <v-autocomplete variant="solo" single-line density="compact" hide-details
                            :items="['主管會報', '主管會報', '主管會報']"></v-autocomplete>
                        </div>
                      </v-col>
                    </v-row>
                  </v-container>
                ...
                <v-btn class="closeSearchMoreBtn" elevation="0"
                  @click="pageInfo.searchForm.expand = !pageInfo.searchForm.expand">
          ...
          <v-card-text>
            <v-window v-model="pageInfo.curTab">
              <v-window-item value="tb1">
                ...
                <v-data-table-server v-bind="pageInfo.dataTable" v-model="pageInfo.dataTable.selected"
                  @update:options="loadItems">

                  <template v-slot:item.action="{ item }">
                    <v-menu open-on-hover>
                      <template v-slot:activator="{ props }">
                        <v-btn density="compact" icon="smi-file-document-edit" size="small" color="success"
                          v-bind="props" />
                      </template>
                      <v-list>
                        <v-list-item v-for="(item, index) in pageInfo.actBtnList" :key="index">
